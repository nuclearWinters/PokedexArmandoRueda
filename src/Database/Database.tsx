      ...
      async (resolve, reject): Promise<void> => {
        try {
          ...
          if (!pokemonsCatched) {
            ...
            const pokemonsCatchedParse: IPokemon[] = JSON.parse(
              pokemonsCatched,
            );
  ...
  getPokemonCatched: (name: string) =>
    new Promise<IPokemon>(
      async (resolve, reject): Promise<void> => {
        try {
          const pokemonsCatched = await AsyncStorage.getItem('pokemonsCatched');
          if (!pokemonsCatched) throw new Error('No tienes ningún pokemon');
          const pokemonsCatchedParse: IPokemon[] = JSON.parse(pokemonsCatched);
          const hasPokemon = pokemonsCatchedParse.find(
            (item) => name === item.name,
          );
          if (!hasPokemon) throw new Error('No se tienes este pokemon');
          resolve(hasPokemon);
        } catch (e) {
          reject(e);
        }
